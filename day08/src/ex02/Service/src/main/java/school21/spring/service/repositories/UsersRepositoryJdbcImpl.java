...
public class UsersRepositoryJdbcImpl implements UsersRepository {
	...
	private final String UPDATE_QUERY_TEMPLATE = "UPDATE Users SET email=?, password=? WHERE id=?";
	private final String INSERT_QUERY_TEMPLATE = "INSERT INTO Users (email, password) VALUES (?, ?)";
	...
	public List<User> findAll() {
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			while (result.next()) {
				lUsers.add(new User(result.getLong("id"), result.getString("email"), result.getString("password")));
			}
	...
	public Optional<User> findById(Long id) {
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			if (result.next()) {
				user = new User(result.getLong("id"), result.getString("email"), result.getString("password"));
			}
		} catch (SQLException e) {
			System.err.println(e.getMessage());
		}
